...
def threeoption_get_reward(state):
...
def threeoption_graph_reward(r_char, n, old_state, sensors):
    ...
    if active_sensors:
        ...
        return -1
badgraphreward = functools.partial(threeoption_graph_reward, 12, 3)
goodgraphreward = functools.partial(threeoption_graph_reward, 32,1)
def threeoption_battery_dynamics(generated_power,battery_capacity, maxbatt, max_t, status, scaledbattery, randomness=False):
    ...
    if status == 2:
        used_power = (deepsleeppower*discharge_voltage*timeperiod)
    if status == 0:#either pre-sleep or awake
...
def threeoption_status_dynamics(status, battery, action):
    ...
    if action == 1: #idle
        new_status = 1
    if action == 2: #deep sleep
        new_status = 2
    return new_status
def threeoption_what_is_noop(state):
    return state[0]
def threeoption_get_new_state(batt_funs, battery_capacity, max_batt,max_t, old_state, action):
    ...
    full_actions = {k:threeoption_what_is_noop(old_state[k]) for k in old_state
                           if k != action_key}
    full_actions[action_key] = action_val

    new_statuses = {k: threeoption_status_dynamics(v[0],v[1],full_actions[k])
    ...
    return new_state
class ThreeOptionSensorEnv(gym.Env):
    ...
    def __init__(self, max_batt, num_sensors, solarpowerrecord,deltat,
        ...
        self.action_space = spaces.Tuple((spaces.Discrete(num_sensors),spaces.Discrete(3)))
        self.deltat = deltat
    ...
    def step(self, action):
        ...
        new_state = threeoption_get_new_state(self.episode_battery_dynamics, 
    ...
    def reset(self):
        ...
        self.episode_battery_dynamics = {k: episode_battery_runner(threeoption_battery_dynamics)
...
class BadThreeOptionSensorEnv(ThreeOptionSensorEnv,gym.Env):
    def step(self, action):
        assert self.action_space.contains(action)
        old_state = self.state
