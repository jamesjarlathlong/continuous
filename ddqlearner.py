...
class DDQNConvAgent:
    def __init__(self, env,n_episodes, max_env_steps=None, modeldir=None, learning_rate = 0.0001, decay_rate = 0.999995, layer_width=64):
        self.env = env
        self.n_episodes = n_episodes
        self.state_size = np.shape(flatten_state_forconv(env.observation_space.sample()))
        self.action_size = get_alt_action_size(self.env)
        print(self.action_size, self.state_size)
        self.action_lookup = functools.partial(alt_action_lookup,env)#list(itertools.product(*(range(space.n) for space in env.action_space.spaces)))
        self.memory = deque(maxlen=2000)
    ...
    def _build_model(self):
        # Neural Net for Deep-Q learning Model
        model = Sequential()
        print(self.state_size, 'state size')
        model.add(Conv1D(self.layer_width, 4, input_shape = (16,4), padding='valid'))
    ...
    def act(self, state):
        #if np.random.rand() <= self.epsilon:
            #return random.randrange(self.action_size)
        act_values = self.model.predict(state.reshape(1,16,4))
        return np.argmax(act_values[0])  # returns action
    def replay(self, batch_size):
        minibatch = random.sample(list(self.memory), batch_size)
        for state, action, reward, next_state, done in minibatch:
            ...
            if not done:
                ...
                target_pred = self.target_model.predict(next_state.reshape(1,16,4))
                target = reward + self.gamma * \
                       np.amax(target_pred[0])
            target_f = self.model.predict(state.reshape(1,16,4))
            target_f[0][action] = target
            self.model.fit(state.reshape(1,16,4), target_f, epochs=1, verbose=0)
        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay
    def run(self, render=True):
        for e in range(self.n_episodes):
