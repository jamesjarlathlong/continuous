...
def twooption_graph_reward(r_char, n, old_state, sensors):
    ...
    dead_sensor_names = [k for k,v in old_state.items() if state[1]==0]
    connected = [k for k,v in connectivity.items() if v]
    connected_and_notdead = [k for k in connected if k in hasbatt_sensor_names]
    capable_rewards =  (len(connected_and_notdead)-len(dead_sensor_names))/(len(sensors))
